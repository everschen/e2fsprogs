			...
			ext2_ino_t *ret_ino)
{
	...
	ECFS_DEBUG("parent:%lld ino=%lld, name=%s, inline_data=%d link_flags=%d", parent, ino, name, inline_data, link_flags);
	/*
	...
	if (!inline_data) {
		ECFS_DEBUG("parent:%lld ino=%lld, name=%s, inline_data=%d", parent, ino, name, inline_data);
